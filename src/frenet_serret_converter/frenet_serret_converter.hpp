...
class FrenetSerretConverter
{
     ...
     *
     * @param mpc_cource vecotr x_ref, y_ref, not used yaw_ref, etc.
     ...
     */
    FrenetCoordinate global2frenet(const MPCCource& mpc_cource, const Pose& pose_g);
     ...
     * @return Pose
     */
    Pose frenet2global(const MPCCource& mpc_cource, const FrenetCoordinate& pose_f) const;
     ...
     */
    int search_nearest_point_index(const int& first_point_index, const int& last_point_index, const MPCCource& mpc_cource, const Pose& pose_g) const;
     ...
     */
    int extract_front_point(const int& nearest_point_index, const MPCCource& mpc_cource, const Pose& pose_g) const;
     ...
     */
    double delta_xf_from_nearest_pt(const int& nearest_point_index, const int& backward_pt_index, const int& forward_pt_index, const MPCCource& mpc_cource, const Pose& pose_g) const;
};
