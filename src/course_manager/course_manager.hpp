...
namespace pathtrack_tools
{
    ...
    private:
        ...
        const double max_curvature_change_rate_ = 0.1;  //!< @brief Saturate value for curvature change rate [1/m^2]
        const double speed_reduction_rate_ = 0.1;       //!< @brief Reduce the speed reference based on the rate of curvature change; v_ref' = v_ref * exp
                                                        //!< (-speed_reduction_rate * curvature_rate^2)
        const double deceleration_rate_for_stop_ = 0.1; //!< @brief Reduce the speed reference for stopping; v_ref'  = v_ref * (1 - exp(-deceleration_rate_for_stop
