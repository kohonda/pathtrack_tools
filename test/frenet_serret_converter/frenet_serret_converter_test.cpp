...
{
    protected:
    // テスト開始前に毎回実行されるFixture
    virtual void SetUp()
    {
        // init_str = "HelloUnitTest!";
        // example_fixture = new ExampleFixture(init_str);
    ...
    {
        // deleteを忘れない用に注意．smart ptrはなぜかエラーがでる
        // もしくは毎回のTESTの中でクラスをコンストラクトする or 初期化関数で初期化するようにする
        delete frenet_serret_converter;
    }

    // 共有される変数
    // std::string ref_path_csv;
...
// TEST_F(FrenetSerretConverterTest, TestGlobal2Frenet_SQUARE)
// {
//     const std::vector<std::array<double, 2>> square_path = {{10, 0.0}, {0.0, 10}, {-10, 0.0}, {0.0, -10}};

//     const std::array<double, 3> pose_g_1 = {5.0, 1.0, 0.0};
//     std::array<double, 3> pose_f_1 = frenet_serret_converter->global2frenet(square_path, pose_g_1);
//     EXPECT_NEAR(5.0, pose_f_1.at(0), 1.0e-6);
//     EXPECT_NEAR(1.0, pose_f_1.at(1), 1.0e-6);
//     EXPECT_NEAR(0.0, pose_f_1.at(2), 1.0e-6);
// }
